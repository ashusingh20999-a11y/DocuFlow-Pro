...
def convert_file(files,target,outdir):
    ...
    if target=="merge_pdf":
        ...
        return out
    if target=="split_pdf":
        if len(files)!=1: raise ValueError("Select one PDF for split")
        r=PdfReader(str(files[0])); out=outdir/"split_pages"; out.mkdir()
        for i,page in enumerate(r.pages,1):
            w=PdfWriter(); w.add_page(page)
            with (out/f"page_{i}.pdf").open("wb") as f:w.write(f)
        zipout=outdir/"split_pages.zip"
        with zipfile.ZipFile(zipout,"w",zipfile.ZIP_DEFLATED) as z:
            for p in out.glob("*.pdf"): z.write(p,p.name)
        return zipout
    if target=="image_pdf":
...
def pdf_xlsx(src,out):
    rows=[]
    for no,page in enumerate(fitz.open(src),1):
...
def docx_pdf(src,out):
    ...
    d=Document(src);c=canvas.Canvas(str(out),pagesize=A4);w,h=A4;y=h-40
...
def pdf_images(src,outdir):
    ...
    for i,p in enumerate(pdf,1):
        ...
        path=outdir/f"page_{i}.jpg";pix.save(path);paths.append(path)
    zipout=outdir/"pdf_images.zip"
    with zipfile.ZipFile(zipout,"w",zipfile.ZIP_DEFLATED) as z:
